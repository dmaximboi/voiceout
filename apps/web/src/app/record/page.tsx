...
import { DURATION_CAP_LABELS, DURATION_CAPS, MAX_POST_IMAGES, type DurationCap } from '@voiceout/shared';
...
function RecordPageInner() {
  ...
  async function publish() {
    if (phaseRef.current === 'posting' || !blob || !caption.trim()) {
    ...
    try {
      ...
      const mediaId = await uploadAudio('post_audio', blob, cap);
  ...
  return (
    <div className="px-4 py-6">
      ...
      <textarea
        ...
        rows={4}
        maxLength={500}
        placeholder="Kindly include a caption — it helps move your post to the people who should hear it"
        value={caption}
        onChange={(e) => setCaption(e.target.value)}
        disabled={phase === 'posting'}
      />
      <p className="mt-1 text-xs text-[var(--muted)]">
        Captions help match language and place so the voice reaches the right feed.
      </p>
      <div className="mt-4">
        {previews.length > 0 ? (
      ...
      <button
        ...
        disabled={!blob || !caption.trim() || locked || live}
      >
